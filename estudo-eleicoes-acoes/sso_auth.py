...
_COOKIE_NAME = "_tag_sso_session"
_COOKIE_MAX_AGE = 86400  # 24 hours
_JWT_LEEWAY = 30  # seconds tolerance for clock skew
...
def validate_sso_token() -> dict | None:
    ...
    try:
        payload = jwt.decode(
            token, SSO_SECRET, algorithms=[SSO_ALGORITHM],
            leeway=_JWT_LEEWAY,
        )
...
def _create_session_token(user_data: dict) -> str:
    """Create a long-lived JWT for cookie-based session persistence."""
    payload = {
        **user_data,
        "exp": int(time.time()) + _COOKIE_MAX_AGE,
    }
    return jwt.encode(payload, SSO_SECRET, algorithm=SSO_ALGORITHM)


def _set_cookie(name: str, value: str, max_age: int):
    """Set a browser cookie via JavaScript injection (height=0, invisible)."""
    from streamlit.components.v1 import html
    html(
        f'<script>document.cookie="{name}={value};path=/;max-age={max_age}'
...
def _read_session_cookie() -> dict | None:
    """Read and validate the session cookie set by _set_cookie."""
    try:
        ...
        required = {"user_id", "email", "nome", "role"}
        if not required.issubset(payload.keys()):
            return None
        return {
...
def require_sso() -> dict:
    ...
    Authentication cascade:
      1. session_state (in-memory, fastest)
      2. sso_token query param (fresh login from portal)
      3. session cookie (survives page reloads / app sleep)
      4. access denied

    The returned dict contains: user_id, email, nome, role
    """
    # 1. In-memory session (fastest path)
    if "sso_user" in st.session_state:
        _inject_watermark(st.session_state["sso_user"])
        return st.session_state["sso_user"]

    # 2. Fresh SSO token from portal URL
    ...
    if payload is not None:
        ...
        # Persist session in a long-lived cookie (24h)
        session_token = _create_session_token(user_data)
        _set_cookie(_COOKIE_NAME, session_token, _COOKIE_MAX_AGE)
        st.query_params.clear()
        _inject_watermark(user_data)
        return user_data

    # 3. Recover session from cookie (survives reload / app sleep)
    cookie_user = _read_session_cookie()
    if cookie_user is not None:
        st.session_state["sso_user"] = cookie_user
        _inject_watermark(cookie_user)
        return cookie_user
